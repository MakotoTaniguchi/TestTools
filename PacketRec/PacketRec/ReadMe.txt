================================================================================
    MFC (MICROSOFT FOUNDATION CLASS) ライブラリ : PacketMonitor プロジェクトの概要
===============================================================================

この PacketMonitor アプリケーションは、アプリケーション ウィザードによって作成されました。このアプリケーションは、Microsoft Foundation Classes の使用の基本を示しているだけでなく、アプリケーションの記述の出発点でもあります。

このファイルには、PacketMonitor アプリケーションを構成する各ファイルの内容の概略が記述されています。

PacketMonitor.vcproj
...
PacketMonitor.h
    これは、アプリケーションのメイン ヘッダー ファイルです。他のプロジェクト固有のヘッダー (Resource.h など) をインクルードしており、CPacketMonitorApp アプリケーション クラスを宣言しています。

PacketMonitor.cpp
    これは、メインのアプリケーション ソース ファイルで、アプリケーション クラス CPacketMonitorApp が含まれています。
...
res\PacketMonitor.ico
    これは、アプリケーションのアイコンとして使用されるアイコン ファイルです。このアイコンは、メイン リソース ファイル PacketMonitor.rc にインクルードされます。

res\PacketMonitor.rc2
...
PacketMonitorDlg.h、PacketMonitorDlg.cpp - ダイアログ
    これらのファイルには、CPacketMonitorDlg クラスが含まれています。このクラスは、アプリケーションのメイン ダイアログの動作を定義します。ダイアログのテンプレートは、Microsoft Visual C++ で編集できる PacketMonitor.rc に含まれています。
...
StdAfx.h, StdAfx.cpp
    これらのファイルは、プリコンパイル済みヘッダー (PCH) ファイル PacketMonitor.pch とプリコンパイル済み型ファイル StdAfx.obj をビルドするために使用されます。

Resource.h
    これは、新しいリソース ID を定義する標準のヘッダー ファイルです。
    このファイルの読み込みおよび更新は、Microsoft Visual C++ で行います。

PacketMonitor.manifest
